...
public class PauseMenuController : MonoBehaviour
{
    ...
    // TMP_Dropdown creates its popup Canvas at sortingOrder 30000.
    // The runtime pause Canvas is intentionally at 32000, so without this
    // repair the popup is rendered behind the pause menu and looks like it
    // never opened. Keep the pause Canvas high and lift only popup layers.
    private Canvas _pauseCanvas;
    private bool _dropdownSortingRepairLogged;
    ...
    {
        if (_page == PausePage.Settings)
        {
            RepairPauseDropdownSorting();
        }
    }
    ...
    private void RepairPauseDropdownSorting()
    {
        ...
        // Keep two layers above the pause shell: blocker first, list second.
        // Canvas sortingOrder is effectively bounded by Unity's UI sorting
        // range, and the current pause shell is 32000.
        int popupOrder = Mathf.Min(32767, _pauseCanvas.sortingOrder + 2);
        int blockerOrder = Mathf.Max(
            _pauseCanvas.sortingOrder + 1,
            popupOrder - 1);
        ...
        for (int index = 0; index < canvases.Length; index++)
        {
            ...
            if (canvas == null || canvas == _pauseCanvas)
            {
                continue;
            }
            ...
            if (objectName == "Dropdown List" ||
                objectName == "Template")
            {
                desiredOrder = popupOrder;
            }
            else if (objectName == "Blocker")
            {
                desiredOrder = blockerOrder;
            }
            else
            {
                continue;
            ...
            if (!canvas.overrideSorting ||
                canvas.sortingLayerID != _pauseCanvas.sortingLayerID ||
                canvas.sortingOrder != desiredOrder)
            {
        ...
        if (repairedAny &&
            !_dropdownSortingRepairLogged)
        {
            _dropdownSortingRepairLogged = true;
            Debug.Log(
                "[PauseMenu] Display dropdown popup sorting repaired " +
                $"above pause Canvas {_pauseCanvas.sortingOrder}.");
        }
    }
