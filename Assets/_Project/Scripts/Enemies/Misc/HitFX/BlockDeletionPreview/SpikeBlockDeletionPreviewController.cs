...
/// V5 makes the target body itself become a block-deletion effect.
/// It does not deal damage and does not touch the formal Spike prefab or AI.
/// </summary>
[DisallowMultipleComponent]
public sealed class SpikeBlockDeletionPreviewController : MonoBehaviour
{
    ...
    [Min(0.16f)] public float adsDuration = 0.275f;
    ...
    {
        float attack = SmoothRange(0f, 0.055f, t);
        float release = 1f - SmoothRange(0.52f, 0.92f, t);
        float error = Mathf.Clamp01(attack * release);
        float sliceEnvelope = Mathf.Sin(Mathf.Clamp01(Mathf.InverseLerp(0.03f, 0.82f, t)) * Mathf.PI);
        float recoil = Mathf.Sin(Mathf.Clamp01(t / 0.34f) * Mathf.PI) * 0.018f;

        ApplyBody(error, sliceEnvelope * 0.82f, 0f, 1f, 1.04f);
        ApplyBodyTransform(
            new Vector3(recoil, -recoil * 0.18f, 0f),
            Quaternion.Euler(0f, 0f, recoil * -112f),
            Vector3.one
        );
        ...
        ApplyGhosts(
            ...
            sliceEnvelope * 0.46f,
            sliceEnvelope * 0.68f,
            new Vector3(0.070f, 0.012f, 0f),
            new Vector3(-0.058f, -0.014f, 0f)
        );

        EvaluateImpactGraphics(t, PreviewMode.Ads);
        EvaluateBlocks(t, 9, 0.040f, 1.08f);
    }
    ...
    {
        bool heavy = mode != PreviewMode.Hipfire;
        bool kill = mode == PreviewMode.Kill;

        float attackEnd = mode == PreviewMode.Hipfire ? 0.075f : 0.060f;
        float releaseStart = mode == PreviewMode.Hipfire ? 0.25f : kill ? 0.45f : 0.36f;
        float releaseEnd = mode == PreviewMode.Hipfire ? 0.73f : kill ? 0.83f : 0.75f;
        float envelope = SmoothRange(0f, attackEnd, t) * (1f - SmoothRange(releaseStart, releaseEnd, t));

        float width = mode == PreviewMode.Hipfire ? 0.86f : mode == PreviewMode.Ads ? 1.18f : 1.42f;
        float height = mode == PreviewMode.Hipfire ? 0.58f : mode == PreviewMode.Ads ? 0.82f : 1.02f;
        float snap = EaseOutBack(Mathf.Clamp01(t / (heavy ? 0.13f : 0.10f)));
        ...
        if (impactBlack != null)
        {
            ...
            SetFx(impactBlack, hardBlack, envelope * 0.96f, 1f);
        }
        ...
        if (impactWhite != null)
        {
            ...
            SetFx(impactWhite, hardWhite, envelope, heavy ? 1.32f : 1.20f);
        }

        if (impactAccent != null)
        {
            float accentDelay = mode == PreviewMode.Hipfire ? 0.095f : 0.070f;
            float local = Mathf.Clamp01((t - accentDelay) / (kill ? 0.54f : 0.43f));
            float accentEnvelope = Mathf.Sin(local * Mathf.PI) * (mode == PreviewMode.Hipfire ? 0.44f : 0.68f);
            ...
            SetFx(impactAccent, _accentColor, accentEnvelope, 1.10f);
        }
    }
    ...
    private void ConfigureBlocks(PreviewMode mode)
    {
        ...
        for (int index = 0; index < blocks.Length; index++)
        {
            ...
            else if (mode == PreviewMode.Ads)
            {
                slot.distance = UnityEngine.Random.Range(0.42f, 0.78f);
                slot.startDelay = UnityEngine.Random.Range(0.00f, 0.075f);
            }
            else
            {
            ...
            {
                sizeBase = UnityEngine.Random.Range(0.28f, 0.46f);
            }
            else
            {
    ...
    private void OnValidate()
    {
        ...
        adsDuration = Mathf.Max(0.16f, adsDuration);
